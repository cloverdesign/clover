...
export const OurWorks = () => {
  ...
  return (
    <section className="flex flex-col w-[100vw] items-center lg:space-y-[140px] pt-[50px] lg:pt-0 space-y-9 relative top-0">
      <motion.h2
        className="absolute text-5xl lg:text-9xl"
        style={{ y }}
        ref={targetRef}
      >
        Our Work.
      ...
      <div className="w-full z-[2] flex flex-col gap-10" ref={containerRef}>
        {WorksData.map((project) => (
          <div
            className="flex flex-col w-full lg:px-12 px-5 space-y-4"
            key={project.id}
          >
            <div className="lg:h-[500px] h-[200px] lg:rounded-[60px] rounded-[30px] overflow-hidden">
              {project.type === "pic" ? (
                <Link href={project.url}>
                  ...
                  >
                    <span className="group-hover:opacity-100 opacity-0 transition duration-500 ease-in-out text-2xl flex items-center justify-center bg-black/40 backdrop-blur-xs h-full w-full font-display">
                ...
                <Link href={project.url}>
                  <motion.div
                    className="w-full h-full hover:cursor-pointer flex items-center justify-center relative group overflow-hidden"
                    initial={{ scale: 1 }}
                  ...
                  >
                    ...
                    />
                    <span className="group-hover:opacity-100 absolute top-0 left-0 opacity-0 transition duration-500 ease-in-out text-2xl flex items-center justify-center bg-black/40 backdrop-blur-xs h-full w-full font-display">
                      <p className="text-white">More about {project.title}</p>
                    </span>
                  </motion.div>
                </Link>
            ...
            <div className="flex flex-wrap items-center justify-between w-full lg:px-12 px-5 gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {project.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="border-[1px] border-white rounded-lg py-1 px-7 uppercase text-[8px] lg:text-xs"
                  >
                    {tag}
                  </span>
                ))}
